...
const PlantCard = ({ plant }) => {
  ...
  // Premium fallback images (luxury botanical photography)
  ...
  return (
    ...
    >
      ...
      <div className="p-3.5 sm:p-4 md:p-5 flex-1 flex flex-col bg-gradient-to-br from-white to-gray-50/50 dark:from-gray-800 dark:to-gray-800/80">
        ...
        <div className="min-h-[3.5rem] sm:min-h-[4rem] md:min-h-[4.5rem]">
          <h3 className="text-sm sm:text-base md:text-lg font-bold text-gray-900 dark:text-white leading-tight line-clamp-2 hover:text-green-600 dark:hover:text-green-400 transition-colors duration-200">
            {plant.name || 'Unnamed Plant'}
          </h3>
        </div>

        {/* Tags Section - FIXED HEIGHT WITH WRAP */}
        <div className="min-h-[3.5rem] sm:min-h-[4rem] mb-3">
          <div className="flex flex-wrap gap-1.5 items-start">
            {plant.section && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/30 dark:to-emerald-900/30 text-green-700 dark:text-green-400 text-xs rounded-full font-medium border border-green-200 dark:border-green-800 shadow-sm whitespace-nowrap">
                <span className="text-green-600 dark:text-green-400 text-[10px]">🌿</span>
                {plant.section.name}
              </span>
            )}
            {plant.category && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30 text-blue-700 dark:text-blue-400 text-xs rounded-full font-medium border border-blue-200 dark:border-blue-800 shadow-sm whitespace-nowrap">
                <span className="text-blue-600 dark:text-blue-400 text-[10px]">🍃</span>
                {plant.category.name}
              </span>
            )}
            {varietyName && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/30 dark:to-pink-900/30 text-purple-700 dark:text-purple-400 text-xs rounded-full font-medium border border-purple-200 dark:border-purple-800 shadow-sm whitespace-nowrap">
        ...
        {/* Stock & Delivery Row - FIXED HEIGHT CONTAINER */}
        ...
        <div className="min-h-[5rem] mb-3">
          ...
          {/* Delivery Badge - Consistent visibility */}
          <div className="mt-2">
            <div className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
              <span className="text-[11px] sm:text-xs">🚚</span>
              <span className="text-[10px] sm:text-xs font-medium text-green-700 dark:text-green-400 whitespace-nowrap">
                Delivery Available
              </span>
        ...
        <div className="flex flex-col gap-2 mt-auto pt-3 border-t border-gray-100 dark:border-gray-700">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={!inStock}
              className={`flex-1 group/btn relative text-xs font-semibold py-2.5 sm:py-3 px-2 rounded-xl transition-all duration-300 flex items-center justify-center gap-1 shadow-md hover:shadow-xl transform active:scale-95 overflow-hidden ${inStock
